function renderProLayout(jsonH, jsonF) {
    ...
    hPosts.forEach(e => {
        ...
        hHtml += `<div class="mn-list-item"><a href="${link}">${e.title.$t}</a></div>`;
    });
...
function initMainNewsHeadlines(catH, catF) {
    ...
    s1.src = `/feeds/posts/default/-/${encodeURIComponent(catH)}?alt=json-in-script&callback=mnCBH&max-results=8`;
